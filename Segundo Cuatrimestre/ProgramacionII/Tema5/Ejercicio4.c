/*
strcmp y leer de fichero las frases
*/
...
void imprimirCadenas(lineas_t lineasFichero);
...
int main (int argc, char **argv){
	...
	if (argc != MIN_MAX_ARG){
		...
		if (existeFichero(argv[1])){
			...
			fclose(fichero);
			printf("Lineas desordenas:\n");
			imprimirCadenas(lineasFichero);
			ordenaCadenas(&lineasFichero);
			printf("Lineas ordenas:\n");
			imprimirCadenas(lineasFichero);
